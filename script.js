(function() {
    ...
    const boringOverlay = document.getElementById('boringOverlay');
    const boringYes = document.getElementById('boringYes');
    const boringNo = document.getElementById('boringNo');

    // 点击中间卡片 → 显示弹窗（每次点击都触发）
    if (centerBox) {
        centerBox.addEventListener('click', () => {
            if (boringOverlay.style.display !== 'flex') {
                boringOverlay.style.display = 'flex';
            }
        });
    }

    // 点“是” → 跳转到自研系统（不记忆，返回后仍可弹出）
    if (boringYes) {
        boringYes.addEventListener('click', () => {
            window.location.href = 'system.html';  // 可替换为实际链接
        });
    }

    // 点“否” → 仅关闭弹窗，下次点击仍会出现
    if (boringNo) {
        boringNo.addEventListener('click', () => {
            boringOverlay.style.display = 'none';
        });
    }
